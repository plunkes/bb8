...
public:
  ...
  FrontierSearch(nav2_costmap_2d::Costmap2D* costmap, double potential_scale,
                 double gain_scale, double orientation_scale,
                 double min_frontier_size, rclcpp::Logger logger);

  /**
   * @brief Runs search implementation, outward from the start position
   * @param position Initial position to search from
   * @param robot_yaw Current robot heading [rad] in the costmap frame, used to
   *        penalise frontiers behind the robot (prefer exploring forwards).
...
private:
  ...
  // Robot pose cached per search, for the orientation (forward-preference) term.
